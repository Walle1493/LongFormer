...
if os.path.exists("/content/drive/MyDrive/external_library"):
    sys.path.append('/content/drive/MyDrive/external_library')
...
# from transformers.modeling_bert_multiqa import BertForMultiQA
# from transformers.modeling_bert import BertForQPMatching

from transformers.optimization import AdamW, get_linear_schedule_with_warmup
# from transformers.tokenization_longformer import LongformerTokenizer, LongformerTokenizerFast
from transformers.modeling_longformer import LongformerForQuestionAnswering
from transformers import RobertaTokenizer, LongformerTokenizer


from utils import dev_evaluate, write_quac, quac_performance, whitespace_tokenize
...
def read_quac_examples(input_file, is_training, max_query_length=64):
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa_idx, qa in enumerate(paragraph["qas"]):
                ...
                if qa_idx > 0:
                    # '{} {} / {}'
                    question_text = '{} {} {}'.format(paragraph['qas'][qa_idx - 1]['question'],
                                                      paragraph['qas'][qa_idx - 1]['orig_answer']['text'],
                                                      qa['question'])
                else:
                    question_text = qa["question"]

                # history_Q.append(qa["question"])
                # question_text = " ".join(history_Q)

                q_tokens = question_text.split()
                if len(q_tokens) > max_query_length:
                    # logger.info('exceed max query before token')
                    question_text = ' '.join(q_tokens[-max_query_length:])
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, doc_stride, max_query_length, is_training):
    ...
    # writer = open("train.pq.txt", "w", encoding='utf-8')
    # writer2 = open("train.a.txt", "w", encoding="utf-8")
    ...
    for (example_index, example) in tqdm(enumerate(examples), total=len(examples)):
        ...
        if len(query_tokens) > max_query_length:
            ...
            for _ in range(max_query_length - len(query_tokens)):
                query_tokens.append("_")
        assert len(query_tokens) == max_query_length
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            # fix span has no answer,but argument [is_impossible] is still 1
            ...
            unique_id += 1
            # qp = " ".join(tokens).replace(" ##", "").replace("##", "").strip()
            # qp = " ".join(qp.split())
            # if example.orig_answer_text == "":
            #     a = "CANNOTANSWER"
            # else:
            #     a = example.orig_answer_text
            # qpa = qp + "ANSWER:  " + a    
            # qpa_list.append(qpa)
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:
                ...
                final_text = get_final_text(tok_text, orig_text, do_lower_case, verbose_logging, vocab_file, merges_file)
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False, \
    vocab_file="longformer-base-4096/vocab.json", merges_file="longformer-base-4096/merges.txt"):
    ...
    # tokenizer = LongformerTokenizer(do_lower_case=do_lower_case)
    tokenizer = LongformerTokenizer(vocab_file, merges_file)
...
class DialogDataLoader:
    def __init__(self, 
        ...
        self.max_batch = max_batch if not self.eval else 100 # control how many qa pairs in one batch
...
def make_dialog_tensors(features, is_eval=False, prv_ctx=2, batch_size=12):
    '''
    ...
    train_data = DialogDataLoader(all_input_ids, all_input_mask, all_segment_ids,
                               ...
                               all_is_impossibles, all_example_indexs, eval_mode=is_eval, prv_ctx=prv_ctx, max_batch=batch_size)
    return train_data
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--longformer_model", default=None, type=str, required=True,
    ...
    #                     help = "The model directory where contains original pretrained model.")
    parser.add_argument("--checkpoints_dir", default=None, type=str, required=True,
                        help = "The model directory where contains checkpoints.")
    ...
    parser.add_argument("--output_file", default=None, type=str,
                        help="QuAC json for output")
    parser.add_argument("--cache_dir", default="cache/", type=str,
                        help="save quac_feature.pt")
    ...
    parser.add_argument("--do_preprocess", default=False, action='store_true', 
                        help="Whether to do data preprocess.")
    parser.add_argument("--do_finetune", default=False, action='store_true',
                        help="choose whether train from origin pretrained model or checkpoints.")
    parser.add_argument("--do_qpmatch", default=False, action='store_true',
                        help="do_qpmatch")
    ...
    parser.add_argument('--log_freq', type=int, default=5000, 
                        help='frequecy to print training process')
    ...
    if n_gpu > 0: # multi-gpu seed initiation
        torch.cuda.manual_seed_all(args.seed)
    
    # check mode and the specified data
    ...
    # prepare tokenizer
    tokenizer = LongformerTokenizer.from_pretrained(args.longformer_model, do_lower_case=args.do_lower_case)

  
    # do_preprocess
    if args.do_preprocess:
        # quac list
        train_examples = read_quac_examples(input_file=args.train_file, is_training=True, max_query_length=args.max_query_length)
        train_features_1 = convert_examples_to_features(
                examples=train_examples[:40000],
                tokenizer=tokenizer,
        ...
        logger.info("Saving train features into file %s quac_train_features.pt", args.cache_dir)
        torch.save(train_features_1, os.path.join(args.cache_dir, "quac_train-%s-1.pt" % args.max_seq_length))
        import gc
        del train_features_1        
        gc.collect()
        ...
        logger.info("Saving train features into file %s quac_train_features.pt", args.cache_dir)
        torch.save(train_features_2, os.path.join(args.cache_dir, "quac_train-%s-2.pt" % args.max_seq_length))
        import gc
        del train_features_2        
        gc.collect()

        # pdb.set_trace()
        eval_examples = read_quac_examples(input_file=args.predict_file, is_training=True, max_query_length=args.max_query_length)
        eval_features = convert_examples_to_features(
                ...
                max_query_length=args.max_query_length,
                is_training=True) 
        logger.info("Saving eval features into file %s quac_eval_features.pt", args.cache_dir)
        torch.save(eval_features, os.path.join(args.cache_dir, "quac_eval-%s.pt" % args.max_seq_length))
    ...
    def run_dev(model, eval_dataloader, evaluator):
        ...
        for batch in eval_dataloader:
            ...
            input_ids, input_mask, segment_ids, context_feature, example_indices = batch
            context_feature = None
            with torch.no_grad():
                batch_start_logits, batch_end_logits, batch_class_logits = model(input_ids, input_mask, segment_ids, context_feature)
            for i, example_index in enumerate(example_indices):
                ...
                all_results.append(RawResult(unique_id=unique_id,
                                            start_logits=start_logits, end_logits=end_logits, class_logits=class_logits))
        pred, nbest_pred = write_predictions(eval_examples, eval_features, all_results,
                                             args.n_best_size, args.max_answer_length,args.do_lower_case, None,
                                             None, None, args.verbose_logging, True, 0.0, ignore_write=True)
        f1 = quac_performance(pred, nbest_pred, evaluator)
        return f1


    if args.do_train:
        train_features_1 = torch.load(os.path.join(args.cache_dir, "quac_train-%s-1.pt" % args.max_seq_length))
        train_features_2 = torch.load(os.path.join(args.cache_dir, "quac_train-%s-2.pt" % args.max_seq_length))
        train_features = train_features_1 + train_features_2
        eval_features = torch.load(os.path.join(args.cache_dir, "quac_eval-%s.pt" % args.max_seq_length))
        train_dataloader = make_dialog_tensors(train_features, prv_ctx=args.prv_ctx, batch_size=args.train_batch_size)
        ...
        status = {}
        if args.do_finetune:
            status = json.load(open(os.path.join(args.checkpoints_dir,"status.json")))
            epoch = status["current_epoch"]
            current_model = os.path.join(args.checkpoints_dir, "checkpoint_{}".format(epoch))
            model = LongformerForQuestionAnswering.from_pretrained(current_model, class_num=1, prv_ctx=args.prv_ctx, use_flow=args.use_flow)
        else:
            model = LongformerForQuestionAnswering.from_pretrained(args.bert_model, class_num=1, prv_ctx=args.prv_ctx, use_flow=args.use_flow)
            status["best_epoch"] = 0
        ...
        # num_train_optimization_steps = int(len(train_features) / args.train_batch_size / args.gradient_accumulation_steps * args.num_train_epochs)
        ...
        for epoch in range(int(args.num_train_epochs)):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss = model(input_ids, input_mask, segment_ids, None, start_positions, end_positions, is_impossible)
                ...
                if (step % 10000 == 0 or step % (len(train_dataloader)-1) == 0) and step!=0:
                    f1 = run_dev(model, eval_dataloader, evaluator)    
                    if f1 >= best_f1:
                        best_f1 = f1
                        status["best_f1"], status["best_epoch"] = best_f1, epoch + 1

                        # save model
                        model_to_save = model.module if hasattr(model, 'module') else model
                        output_model_dir = os.path.join(args.checkpoints_dir, "checkpoint_{}".format(epoch+1))
                    ...
                    with open(os.path.join(args.checkpoints_dir, 'status.json'), "w") as writer:
                        json.dump(status, writer)   

    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        if not os.path.join(args.checkpoints_dir, "status.json"):
            raise ValueError("At least one finetune model should provide.")
        ...
        current_model = os.path.join(args.checkpoints_dir, "checkpoint_{}".format(epoch))
        model = LongformerForQuestionAnswering.from_pretrained(current_model, class_num=1, prv_ctx=args.prv_ctx, use_flow=args.use_flow)
        # model = BertForQPMatching.from_pretrained(current_model)
        model.to(device)
        ...
        for batch in tqdm(eval_dataloader):
            ...
            for i, example_index in enumerate(example_indices):
                ...
                eval_feature = eval_features[example_index.item()]
                unique_id = int(eval_feature.unique_id)
                all_results.append(RawResult(unique_id=unique_id, start_logits=start_logits, end_logits=end_logits, class_logits=class_logits))
